...
async function seedDB(){
    ...
    const hashedPassword = bcrypt.hashSync(process.env.SUPER_ADMIN_PASSWORD, saltRounds)

    const newSuperAdmin = {
        username: ROLES.SuperAdmin,
...
async function cleanUpDB(admin){
    ...
    try {
        ...
        console.log(`${chalk.red('x')} ${chalk.red('error while cleaning database')}`);
        process.exit(1)
    }
}
